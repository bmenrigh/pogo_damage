...
void check_known_cps(double (*cpm_func)(int,int)) {
    ...
    check_specific_cp(cpm_func, 5, 1, "Cetitan", 456, 208, 123, 347, 15, 12, 6);
    check_specific_cp(cpm_func, 6, 1, "Galvantula", 395, 201, 128, 172, 15, 14, 14);
    check_specific_cp(cpm_func, 6, 1, "Eevee", 183, 104, 114, 146, 10, 14, 13);
    check_specific_cp(cpm_func, 7, 1, "Cryogonal", 526, 190, 218, 190, 2, 5, 5);
    check_specific_cp(cpm_func, 8, 1, "Sandshrew (Alola)", 273, 125, 129, 137, 5, 4, 11);
    check_specific_cp(cpm_func, 9, 1, "Seel", 241, 85, 121, 163, 8, 14, 11);
    check_specific_cp(cpm_func, 10, 1, "Cetoddle", 338, 119, 80, 239, 7, 5, 2);
    check_specific_cp(cpm_func, 10, 1, "Fennekin", 291, 116, 102, 120, 9, 14, 13);
    check_specific_cp(cpm_func, 11, 1, "Granbull", 767, 212, 131, 207, 5, 6, 10);
    ...
    check_specific_cp(cpm_func, 17, 1, "Whiscash", 1037, 151, 141, 242, 15, 15, 15);
    ...
    check_specific_cp(cpm_func, 21, 1, "Cetitan", 1812, 208, 123, 347, 9, 8, 15);
    check_specific_cp(cpm_func, 21, 1, "Gulpin", 456, 80, 99, 172, 6, 10, 4);
    check_specific_cp(cpm_func, 21, 1, "Swalot", 1145, 140, 159, 225, 13, 5, 7);
    check_specific_cp(cpm_func, 21, 1, "Sneasel", 1083, 189, 146, 146, 0, 5, 2);
    ...
    check_specific_cp(cpm_func, 30, 1, "Cryogonal", 2253, 190, 218, 190, 7, 2, 14);
    ...
    check_specific_cp(cpm_func, 34, 1, "Rhyperior", 3434, 241, 190, 251, 15, 15, 14);
    ...
    check_specific_cp(cpm_func, 39, 1, "Spheal", 854, 95, 90, 172, 8, 9, 9);
    ...
    check_specific_cp(cpm_func, 40, 1, "Mankey", 1050, 148, 82, 120, 12, 4, 7);
    ...
    check_specific_cp(cpm_func, 42, 1, "Darmumaka", 1399, 153, 86, 172, 10, 10, 13);
    ...
    check_specific_cp(cpm_func, 44, 1, "Swalot", 1838, 140, 159, 225, 4, 4, 4);
    ...
    check_specific_cp(cpm_func, 46, 1, "Stantler", 2262, 192, 131, 177, 12, 14, 8);
    ...
    check_specific_cp(cpm_func, 49, 1, "Seel", 1006, 85, 121, 163, 14, 4, 5);
    check_specific_cp(cpm_func, 49, 1, "Seel", 992, 85, 121, 163, 8, 9, 15);
    check_specific_cp(cpm_func, 49, 1, "Weezing", 2427, 174, 197, 163, 9, 7, 12);
    check_specific_cp(cpm_func, 49, 1, "Patrat", 854, 98, 73, 128, 15, 10, 12);
...
void check_specific_cp(double (*cpm_func)(int,int), int l, int half, const char *name, int truecp, int atk, int def, int hp, int a_iv, int d_iv, int h_iv) {
    ...
    int foundcp;
    if (half == 0) {
        cp = cp_double_2((*cpm_func)(l, 0), atk + a_iv, def + d_iv, hp + h_iv);
    } else {
        cp_l = cp_double_2((*cpm_func)(l, 0), atk + a_iv, def + d_iv, hp + h_iv);
        cp_h = cp_double_2((*cpm_func)(l + 1, 0), atk + a_iv, def + d_iv, hp + h_iv);

        /*cp = ceil((floor(cp_l) + floor(cp_h)) / 2.0);*/
        cp = floor((cp_h + cp_l) / 2.0);
    }
...
void add_true_cp(int l, int half, const char *name, int truecp, int atk, int def, int hp, int a_iv, int d_iv, int h_iv) {
    ...
    int min_down = 0;
    while ((int)floor(cp_double_2(min_cpm, atk + a_iv, def + d_iv, hp + h_iv)) < truecp) {
        min_cpm = adjacent_double(min_cpm, 1);
        min_down--;
    }
    while ((int)floor(cp_double_2(adjacent_double(min_cpm, -1), atk + a_iv, def + d_iv, hp + h_iv)) == truecp) {
        min_cpm = adjacent_double(min_cpm, -1);
        min_down++;
    }

    int max_up = 0;
    while ((int)floor(cp_double_2(max_cpm, atk + a_iv, def + d_iv, hp + h_iv)) > truecp) {
        max_cpm = adjacent_double(max_cpm, -1);
        max_up--;
    }
    while ((int)floor(cp_double_2(adjacent_double(max_cpm, 1), atk + a_iv, def + d_iv, hp + h_iv)) == truecp) {
        max_cpm = adjacent_double(max_cpm, 1);
        max_up++;
    }
